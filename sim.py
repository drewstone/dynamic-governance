...
class Simulator(object):
    def __init__(self, options):
        ...
        if self.agent_type == "honest-increasing":
            ...
            raise new ValueError("Unsupported agent type: {}".format(self.agent_type))

    def start(self):
        if self.logging_mode == "debug": print("Agents = {}".format(
    ...
    def step(self):
        ...
        if self.step_type == "uniform-leader-election":
            leader = self.agents[np.random.choice(np.arange(0, len(self.agents)))]
        ...
        # sample a leader proportional to an agent's capacity
            ...
            leader_index = np.random.choice(np.arange(0, len(caps),p=distribution))
        ...
        # elicit reports from all agents
            ...
            reports = list(map(lambda a: a.report(self.gov.parameter), self.agents))
            # advance round and receive new parameter given reports
            self.gov.advance_round(reports)
        else:
            raise ValueError("Unsupported step type: {}".format(self.step_type))
